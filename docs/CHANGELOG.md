...
- Removed the GitHub contribution heatmap: the device TLS layer could not complete RSA signature verification for any chain, and the feature is parked until that is root-caused. Removal frees ~165 KB of flash (the app is now 34%% free). In its place, the pet card became a real Tamagotchi-style pixel pet: an egg hatches after two minutes into baby, child, and adult stages (black-and-white bitmap sprites with two-frame idle animation); hunger, mood, cleanliness, and energy decay over real time with offline settlement capped at three days; droppings appear periodically and need cleaning; sleeping restores energy. In the pet layout the arrow keys choose an action (feed/play/clean/sleep-wake) and OK performs it; state persists in NVS and survives reboots. Home layouts are now three (card / QR codes / pet).
