...
- 宠物新增拓麻歌子式小游戏:动作模式里选「玩耍」现在会打开小游戏菜单,内置两个经典——**猜一猜**(宠物躲进两扇门之一,▲/▼ 猜左/右后探头揭晓,5 局,拓麻歌子传统项目)和**快反应**(苹果随机时刻出现,900ms 内按 OK 算赢,提前按算抢跑)。赢局加心情(每胜 +1g 体重),开局耗 10 点精力,太累的宠物会拒绝玩耍。比分实时显示,结果随宠物状态持久化。调试期约定:进入宠物页时四项数值自动拉满并唤醒宠物,动作与小游戏随时可测(后台衰减循环照常运行;想恢复真实衰减随时说)。
