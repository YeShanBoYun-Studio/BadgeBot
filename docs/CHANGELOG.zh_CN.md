...
- 配网改为「重启进门户」:进入配网页时打 RTC 标记并重启,开机消费标记直接落在配网页。门户与其他功能共用 ~26KB 堆,而反复开关会实测性地把堆切碎(三轮循环门户空闲堆 26.5→26.3→25.9KB 衰减,第二轮起页面要 20 秒且截断);重启让每次配网都在 pristine 堆上进行——实测 34.6KB 空闲/26.6KB 最大连续块(不重启只有 25.8KB/14.3KB)。硬件自动化验收:连续 5 次页面抓取 63-89ms、页面 JavaScript 校验完整、config 与 notes 写读回环、断开重连后 82ms。堆遥测新增最大连续块指标;串口命令通道(p 开门户/x 关闭/P 重启进门户)让 PC 无需触碰徽章即可驱动配网测试。另:LVGL 池 64→48KB——四个 96px 二维码画布放得下且有余量(QR 建屏时打印池用量),把 16KB 还给门户急需的系统堆。

- 修复配网门户"实际不可用"的问题——开启门户期间的堆轨迹日志(每 5 秒一拍)定位出五个叠加的因:HTTP 服务器的 URI 处理器表(8)小于门户的 14 条路由,最后 4 条(二维码 2/3 槽与提词稿读写)静默 404;手机联网探测失败会把网络判为"无互联网",随即深度休眠网卡并把浏览器切回蜂窝,导致 http://192.168.4.1 永远打不开(探测端点现返回 204/"Success",未匹配路径 302 跳门户);19KB 的门户页面单次发送在 softAP 链路上中途夭折,得到"表单都在但按钮全死"的页面(尾部 `<script>` 没送到)——页面现以 gzip 压缩 7.3KB 整体下发(源文件迁至 `main/portal.html`,由 `tools/gen_portal_gzip.py` 生成 `portal_html_gz.h`,validate 门禁强制检查同步);「完成并联网」3 秒后关闭热点,逼着用户每传一个文件重新配一次网(现在它只触发门户内即时校时——AP+STA 并存——热点保持开放到 180 秒窗口结束,期间随时可重连);以及手机关联本身会把堆打到 204 字节的最低水位(单连接 TCP 缓冲减半至 2920B、WiFi TX 池减半至 16、HTTP 并发限 3 连接 + LRU 淘汰、softAP 固定 11 信道、AP 服务期间关闭调制解调器省电)。同时新增 AP 侧事件日志(探针/关联/断开及原因码)——其余问题都是靠它定位的。

- 新增语音输入(V2 架构:徽章只是无线麦克风,识别与打字都在电脑侧)。在新的「语音」页按 OK 会按需联网,并把麦克风 16kHz/16bit 单声道 PCM 以 chunked HTTP 直传电脑上的小型 Python 后端(`tools/badge_voice.py`);再按 OK(或录满 30 秒自动)结束,后端识别文字、直接打进电脑当前聚焦的窗口,徽章屏上同步显示识别文本。内置三种引擎:`echo`(回环测试)、任意 OpenAI 兼容的 `/audio/transcriptions` 服务(纯标准库 multipart——OpenAI、Groq、硅基流动等)、本地 `faster-whisper`;识别在电脑侧完成,所以中文可用,徽章侧既不碰 TLS(设备有已知 RSA 验签问题)也不碰蓝牙,严守射频互斥。后端地址在门户新增的「语音」区配置,持久化到 NVS(`voice_url`);按需联网走新增的 Wi-Fi hold API,一轮结束后无线电交还给整点脉冲。后端加固:服务地址仅放行 http(s)、禁止 userinfo、禁用重定向。应答解析与地址拼接是带主机测试的纯逻辑模块;后端的 chunked 路径用 echo 引擎做了回环测试。

- 修复宠物页在时钟从未校准(刚刷机或没配 WiFi)时整体装死的问题:动作和小游戏此前被"已校时"门槛拦住,而它们其实不依赖时钟——真正需要时钟的只有衰减结算。现已去掉这些门槛;未校时时成长阶段改为按已结算的宠物年龄推算,不再把已出生的宠物显示回成蛋;心情同样改回由数值推导而非显示"--"。新增未校时阶段回退的主机测试用例。

- 宠物新增拓麻歌子式小游戏:动作模式里选「玩耍」现在会打开小游戏菜单,内置两个经典——**猜一猜**(宠物躲进两扇门之一,▲/▼ 猜左/右后探头揭晓,5 局,拓麻歌子传统项目)和**快反应**(苹果随机时刻出现,900ms 内按 OK 算赢,提前按算抢跑)。赢局加心情(每胜 +1g 体重),开局耗 10 点精力,太累的宠物会拒绝玩耍。比分实时显示,结果随宠物状态持久化。

- 新增提词器(M5,与翻页联动)。讲稿来自门户新增的「提词稿」区块:UTF-8 纯文本,**一行 = 一段(对应 PPT 一页)**,存为 store 分区的 `notes.txt`(上限 32KB)。三种喂法共用一条链路:直接粘贴 / 上传 .txt / 上传 .pptx 自动提取每页备注——提取在手机浏览器里完成(手写 ZIP 目录解析 + 浏览器内置 DecompressionStream 解压 `ppt/notesSlides/notesSlideN.xml` 并收集 `<a:t>` 文本,零第三方依赖),配网热点无外网也能用,固件端零文档解析。板子上菜单新增「提词器」页:大字显示当前段 + i/N 进度条;与翻页器共用同一 BLE 键盘——联动模式下 ▲/▼ 给电脑发上/下页并同步翻本地段,PPT 与提词稿天然对齐(电脑零软件);自动模式下文字按可调速度连续滚动、一段滚完自动进下段(▲/▼ 调速,OK 双击暂停)。恢复出厂现在也会清除讲稿。分段逻辑为纯模块并带主机测试;pptx 提取链路在 Node 中用生成的 .pptx 做了端到端验证。另修复本周期早前引入的门户 JS 语法错误(丢失字符串拼接运算符),该错误会令整个配网页面失效。

- 修复翻页器页显示 "BLE failed":蓝牙主机约需 40KB 堆,而 Wi-Fi 驱动初始化后堆只剩约 38KB,NimBLE 每次启动都失败。按原设计的「无线栈互斥」方案,进入翻页器页时挂起 Wi-Fi 脉冲(停驱动并 deinit 释放内存),退出页面时重新初始化 Wi-Fi 并恢复每小时脉冲;脉冲循环可被挂起请求提前打断,挂起等待约 1 秒。已在真机验证蓝牙以「BadgeBot」成功广播。

- 修复时钟显示 UTC 而非本地时间(北京时间 13:15 板子显示 05:15;日期恰好同日所以看着正常):代码在 `app_clock_init()` 里设置了 `TZ=CST-8`,但该函数从未被调用。现在开机即调,校时日志同时打印换算后的本地时间与时区便于核验。连带修复菜单里「翻页器」显示 [FAIL] 且无法进入的问题(就绪标志漏置位)。

- 新增 BLE 翻页器(M3):工牌以蓝牙键盘「BadgeBot」身份广播(HID-over-GATT 服务在 NimBLE 上自实现,IDF 5.5 没有官方 HID 设备例程)。电脑蓝牙设置里配对一次后,▲/▼ 即发送 PgUp/PgDn,演示文稿、PDF、看图翻页全程电脑零软件;有宿主连接时状态栏蓝牙图标点亮。翻页器页 OK 短按切换键位映射:PgUp/PgDn(演示文稿)↔ 左/右方向键(看图/漫画),选择持久化到 NVS(`hidmap` 键)。配对为 Just Works + LE 安全连接,绑定密钥持久化在 NVS,工牌重启后电脑自动回连;设备侧擦过密钥时按「重复配对」流程删旧绑定重建。

- 重构宠物页按键,动作真正可用:宠物页 OK 长按现在进入/退出「动作模式」(高亮提示,动作行显示当前选中),而此前长按会进菜单、短按也进菜单,动作永远执行不到,页面还容易被"卡住"。普通模式下 ▲/▼ 切布局、OK 短按开菜单(与其他页面一致);动作模式下 ▲/▼ 选 喂食/玩耍/清洁/睡觉-叫醒,OK 执行。

- 设置页新增「恢复出厂」行(替换原固件版本行):第一次 OK 进入待确认(显示「再按OK确认」),第二次 OK 执行——擦除 NVS 配置与宠物数据、删除上传的头像/二维码图、主题回默认并重建页面;移到其他行即取消。

- 二维码布局扩为 4 槽(2×2 网格,每槽 96px)。每槽可在门户里独立选择「网页生成」(设备端渲染文本)或「上传图片」(二值化为 128×128,存为 `qr0.img`…`qr3.img`),槽下显示各自说明文字(缺省 QR1…QR4)。门户新增四槽表单(每槽:说明、生成/上传单选、文本框或裁剪上传),新增 `/api/qr0`…`/api/qr3` 上传端点;配置 API 改为 `qr_text`/`qr_label` 数组加 `qr_mode` 位图。LVGL 内存池 48→64KB,保证 4 个设备生成码同时放得下。

- 移除 GitHub 提交热力图:设备 TLS 层对所有证书链的 RSA 验签都无法完成,该功能在根因查明前先下线,释放约 165KB 固件空间(应用余量升至 34%)。取而代之,宠物卡片升级为真正的拓麻歌子式像素宠物:蛋 2 分钟孵化,依次成长为幼年/少年/成年(黑白点阵精灵,双帧待机动画);饱食/心情/清洁/精力随真实时间衰减,离线也会结算(封顶 3 天);会周期性拉便便需要清理;睡觉恢复精力。状态存 NVS,重启不丢。主页布局现为三种(名片/二维码/宠物)。

- 修复头像上传报“写入失败”：FATFS 长文件名支持此前未启用，而 96×96 头像文件名超出 8.3 短名限制（二维码图文件名恰好合规，所以只有头像失败）。配网门户新增手动正方形裁剪编辑器（选框可拖动/缩放，默认居中）；新增 10 秒心跳行显示热点剩余时间，设备不可达时变红；所有操作的成败回执按绿/红着色。

- 热力图拉取改为低内存模式：公开 contributions 接口对单个用户返回约 255KB，现在用 2KB 滚动块流式解析成 371 字节的 NVS 缓存，不再整体缓冲；拉取前先等 SNTP 校时完成，失败自动重试一次。主页大缓冲改为按布局精确分配。已知问题：当前设备对所有 TLS 链的服务器证书验证都确定性失败（RSA 运算层面），热力图在根因查明前会保持占位页；失败是优雅降级，已缓存数据仍可渲染。

- 配网门户在保存 Wi-Fi 后不再关闭热点，头像/二维码上传与资料修改可以继续；页面底部新增醒目的「完成并联网」按钮，点击后关闭热点并立即联网校时、拉取热力图。所有表单操作都有可见回执（成功绿色、失败红色），图片解码失败也会提示；页面新增中/英文切换。上传保持严格的居中裁成正方形（头像 96×96、二维码图 128×128 黑白），并在文案中说明。

- 设备界面新增语言设置（中文/English），覆盖菜单、设置项与取值、按键提示和各页静态文案（用户输入的内容始终按原文显示）。GB2312 字库的符号表改为自动收录界面源码中出现的全部非 ASCII 字符，修复了缺字渲染为空白的问题（如“浏”）。

- 工牌主页新增第四套布局“宠物”卡片（大号吉祥物+心情占位），与名片、双二维码、GitHub 热力图并列。设置页新增布局子页，可逐个开启/关闭每套布局（至少保留一个）；主页布局切换只在开启的布局间循环。热力图布局在设备拉到数据后会自动重绘。

- 扩充配网门户与工牌主页。热点每次会话使用随机的 SSID 和密码（扫屏幕上的二维码即可入网，无需手输）；门户表单新增手机端图片上传：名片头像（居中裁切为 96×96）和第二张二维码图（如微信名片，二值化为 128×128），并增加 GitHub 用户名输入；上传资产存入 FATFS `store` 分区。工牌主页支持三种可切换布局：名片（上传头像或吉祥物，姓名/公司/岗位）、双二维码（由配置链接生成的码 + 上传的图片）、GitHub 提交热力图（53×7）——设备在 Wi-Fi 脉冲窗口内自行从公开 contributions 接口拉取并缓存到 NVS。时钟只在名片布局显示。

- 联网修复：设备开机即打一次 Wi-Fi 脉冲，不再等满一小时，校时与热力图在开机后即可刷新；裁剪 Wi-Fi 驱动 RX 缓冲（静态 10→6、动态 32→16），主页大图片缓冲从常驻静态改为按需分配，修复开机时 `esp_wifi_init` 内存不足导致设备一直不联网的问题（初始化失败现在会重试而不是放弃）。

- 重做共享 UI：每个屏自带顶部状态栏（电量、音量、Wi-Fi、蓝牙图标）和底部按键提示条，按键帽粗边代表长按、细边代表短按。新增浅色/深色两套主题（语义化调色板角色、存 NVS、设置页可切换，后续可扩展），设置页与按键提示使用中文，由新增的 16px CJK 字体渲染；全局自动息屏（任意键亮屏，唤醒键不分发）、主页长按 ▼ 立即息屏；菜单翻页不再重建屏幕。修复删屏后吉祥物无限眨眼动画残留导致的悬空动画（菜单机器人乱跑的根源）。

- 新增工牌主页与设置页。主页显示配置的姓名、公司与岗位（可隐藏）、电量条和时间占位，并在设定的空闲时间后自动关闭背光；设置页可调亮度、音量、主页布局、息屏时间与开机页，全部持久化到 NVS。用户文字用新增的 20 px、1 bpp CJK 位图字体（ASCII + GB2312 一级汉字）渲染。菜单支持分页，设备默认开机进入工牌主页。
